...
-- 8. Procedimiento para eliminar autor (Agregado)
DELIMITER //
CREATE PROCEDURE sp_EliminarAutor(IN p_id INT)
BEGIN
    -- Verificar si el autor existe
    DECLARE autor_count INT DEFAULT 0;
    SELECT COUNT(*) INTO autor_count FROM autores WHERE id = p_id;

    IF autor_count = 0 THEN
        SELECT CONCAT('Error: No existe el autor con ID ', p_id) AS resultado;
    ELSE
        -- (Opcional) Verificar si el autor tiene libros asociados
        -- IF EXISTS (SELECT 1 FROM libros WHERE autor_id = p_id) THEN
        --     SELECT 'Error: No se puede eliminar un autor que tiene libros asociados' AS resultado;
        -- ELSE
            DELETE FROM autores WHERE id = p_id;
            SELECT CONCAT('Autor ID ', p_id, ' eliminado correctamente') AS resultado;
        -- END IF;
    END IF;
END //
DELIMITER ;

-- 9. Procedimiento para obtener estadísticas
DELIMITER //
...
-- 10. Procedimiento para buscar libros por título
DELIMITER //
...
-- 12. Procedimiento para insertar reseña
DELIMITER //
